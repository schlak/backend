...
// -> 0 */12 * * * : every 12 hours
// -> 0 0 * * *    : at 00:00
const reIndex = new cron.CronJob("0 0 * * *", function() {
    populateIndexAndMetadata();
...
function populateIndexAndMetadata() {
    ...
    // Cache
    // const cachedIndex = cache.get("metadata.json", []);
    ...
    musicIndex.forEach((track, i) => {
        metadata.basic(track.path).then((trackMetadata) => {
            ...
            const albumId = crypto.createHash("sha1")
                .update(trackMetadata.album + trackMetadata.album_artist)
                .digest("hex");

            if (!cache.exists(`${albumId}/image.jpg`)) {
                metadata.cover(track.path).then((cover) => {
                    if (cover) {
                        cache.add(
                            cover.data,
                            "image.jpg",
            ...
            // Cache tracks index + metadata
            if (musicIndex.length === i + 1) {
                cache.add(JSON.stringify(musicIndex), "metadata.json", ".");
            }
        });
    });
}
